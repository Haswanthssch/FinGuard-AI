...
MODEL_PATH   = os.path.join(str(settings.MODELS_DIR), "risk_classifier.pkl")
ENCODER_PATH = os.path.join(str(settings.MODELS_DIR), "risk_label_encoder.pkl")
...
def load_risk_classifier():
    ...
    model = bundle
    # If it's legacy, it might need joblib to load the separate encoder
...
def predict_risk(metrics: dict) -> dict:
    ...
    if le:
        ...
        categories = ["Conservative", "Moderate", "Aggressive", "Speculative"]
        label = categories[int(pred_idx)] if int(pred_idx) < len(categories) else str(pred_idx)
        class_proba = {f"Class {i}": round(float(p), 4) for i, p in enumerate(proba)}
